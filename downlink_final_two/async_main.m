...
for s=(historical_samples_req+1):num_samples   %the iterator s accounts for historical occupancy matrix
   ...
   %transmit to
    ...
    if count1 == 1 && ~(ap.interface_one.state == 3) && ~(ap.interface_one.state == 5)
        ap.interface_one.previous_state = ap.interface_one.state;
        ap.interface_one.state = 5;
    end

    if count2 == 1 && ~(ap.interface_two.state == 3) && ~(ap.interface_two.state == 5)
