function [ap, sta, time] = update_slo_mlo_flows_dynamic(ap, sample_no, sta, time, historical_occupancy_matrix)
   ...
   mlo_umac = ap.mlo_umac;
    ...
    if rem(sample_no, s_time_period_allocation) == 0 || mlo_flow_arrives  %calculate flow and allocate it to links
        ...
        if mlo_flow_arrives
            ...
            for i = n_active_lmac_mlo_flows + 1 : n_active_umac_mlo_flows
               ...
               interface_two.mlo_lmac.flow_details(i).flow = mlo_umac.flow_details(i).flow; %not sure if this loop req or not
        ...
        %channel_occupancy
        if ap.channel_occupancy(1) ~= channel_occupancy(1) || ap.channel_occupancy(2) ~= channel_occupancy(2) || mlo_flow_arrives 
            
            %if channel_occupancy(1) >= ap.soft_threshold(1) || channel_occupancy(2) >= ap.soft_threshold(2) || mlo_flow_arrives 
                ...
                [ap.soft_threshold, ap.hard_threshold, mlo_load_assign, ap.percentage_available_channel, n_int, app_flow] = calculate_data_rate_for_two_links(ap.channel_occupancy, ap.soft_threshold, ap.hard_threshold, round_no, ap.percentage_available_channel, n_int, ap.rate_mcs_latest, mlo_umac);
                %[ap.soft_threshold, ap.hard_threshold, mlo_load_assign, ap.percentage_available_channel, n_int, app_flow] = calculate_data_rate_for_two_links_SLCI(channel_occupancy, ap.soft_threshold, ap.hard_threshold, round_no, ap.percentage_available_channel, n_int, ap.rate_mcs_latest, mlo_umac);
                time = time + toc;
